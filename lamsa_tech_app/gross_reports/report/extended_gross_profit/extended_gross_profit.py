...
# An extend version of the original Gross Profit report that's shipped with
# ERPNext with an additional filter an "Group By" option
# "selling price list"
...
def execute(filters=None):
    ...
    if filters.group_by == "Invoice":
        ...
        get_data_when_not_grouped_by_invoice(
            gross_profit_data, filters, group_wise_columns, data)
...
class GrossProfitGenerator(object):
    ...
    def process(self):
        ...
        self.currency_precision = cint(
            frappe.db.get_default("currency_precision")) or 3
        self.float_precision = cint(
            frappe.db.get_default("float_precision")) or 2

        grouped_by_invoice = True if self.filters.get(
            "group_by") == "Invoice" else False

        if grouped_by_invoice:
            buying_amount = 0

        # row is sales invoice item with additional details from joins like invoice info
        # sales, ..etc
        for row in reversed(self.si_list):
            ...
            product_bundles = []
            if row.update_stock:
                product_bundles = self.product_bundles.get(
                    row.parenttype, {}).get(row.parent, frappe._dict())
            elif row.dn_detail:
            ...
            if row.item_code in product_bundles:
                ...
                row.buying_amount = flt(self.get_buying_amount(
                    row, row.item_code), self.currency_precision)
            ...
            if flt(row.qty):
                row.buying_rate = flt(
                    row.buying_amount / flt(row.qty), self.float_precision)
                row.base_rate = flt(
                    row.base_amount / flt(row.qty), self.float_precision)
            else:
            ...
            self.grouped.setdefault(
                row.get(scrub(self.filters.group_by)), []).append(row)
        ...
        for key in list(self.grouped):
            if self.filters.get("group_by") == "Invoice":
                ...
                for i, row in enumerate(self.grouped[key]):
                    ...
                    if i == 0:
                        new_row = row
                        self.set_average_based_on_payment_term_portion(
                            new_row, row, invoice_portion)
                    else:
                        new_row.qty += flt(row.qty)
                        self.set_average_based_on_payment_term_portion(
                            new_row, row, invoice_portion, True)
                ...
                for i, row in enumerate(self.grouped[key]):
                    if i == 0:
                        ...
                        new_row.qty += flt(row.qty)
                        new_row.buying_amount += flt(row.buying_amount,
                                                     self.currency_precision)
    ...
    def set_average_rate(self, new_row):
        ...
        new_row.buying_rate = (
            flt(new_row.buying_amount / new_row.qty,
                self.float_precision) if new_row.qty else 0
        )
        new_row.base_rate = (
            flt(new_row.base_amount / new_row.qty,
                self.float_precision) if new_row.qty else 0
        )
        return new_row
    ...
    def set_average_gross_profit(self, new_row):
        ...
        new_row.gross_profit_percent = (
            flt(((new_row.gross_profit / new_row.base_amount)
                * 100.0), self.currency_precision)
    ...
    def get_buying_amount_from_product_bundle(self, row, product_bundle):
        ...
        for packed_item in product_bundle:
            if packed_item.get("parent_detail_docname") == row.item_row:
                ...
                buying_amount += self.get_buying_amount(
                    packed_item_row, packed_item.item_code)
    ...
    def calculate_buying_amount_from_sle(self, row, my_sle, parenttype, parent, item_row, item_code):
        for i, sle in enumerate(my_sle):
            ...
            if (
                ...
                previous_stock_value = len(
                    my_sle) > i + 1 and flt(my_sle[i + 1].stock_value) or 0.0
    ...
    def get_buying_amount(self, row, item_code):
        ...
        if item_code in self.non_stock_items and (row.project or row.cost_center):
            ...
            if (row.update_stock or row.dn_detail) and my_sle:
                ...
                my_sle = self.get_stock_ledger_entries(
                    item_code, row.warehouse)
                return self.calculate_buying_amount_from_sle(
                    row, my_sle, parenttype, parent, item_row, item_code
                )
            elif row.sales_order and row.so_detail:
                incoming_amount = self.get_buying_amount_from_so_dn(
                    row.sales_order, row.so_detail, item_code)
                if incoming_amount:
    ...
    def load_invoice_items(self):
        ...
        if self.filters.item_group:
            conditions += " and {0}".format(
                get_item_group_condition(self.filters.item_group))
        ...
        if self.filters.get('selling_price_list'):
            conditions += 'and `tabSales Invoice`.selling_price_list = %(selling_price_list)s'
        ...
        self.si_list = frappe.db.sql(
			...
			select
				...
				`tabSales Invoice`.territory,  `tabSales Invoice`.selling_price_list,
    ...
    def get_delivery_notes(self):
        ...
        if self.si_list:
            ...
            for entry in delivery_notes:
                self.delivery_notes[(entry.sales_invoice,
    ...
    def get_bundle_item_row(self, product_bundle, item):
        item_name, description, item_group, brand = self.get_bundle_item_details(
            item.item_code)
